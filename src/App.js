...
const CSS = `
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  @media(max-width:768px){
    ...
    .sidebar{width:100%;height:56px;flex-direction:row;position:fixed;bottom:0;top:auto;z-index:100;border-right:none;border-top:1px solid rgba(255,255,255,.1)}
    .sidebar>div:first-child{display:none}
    .sidebar nav{flex-direction:row;padding:0;flex:1;overflow-x:auto}
    .ni{flex-direction:column;font-size:9px;gap:2px;padding:8px 12px;border-left:none;border-top:2px solid transparent;min-width:60px;justify-content:center;align-items:center}
    .ni.on{border-left:none;border-top-color:#4ade80}
    .sidebar>div:last-child{display:none}
    .main{margin-bottom:56px;height:calc(100vh - 56px)}
    .msg-layout{flex-direction:column}
    .contacts{width:100%;max-height:35vh;border-right:none;border-bottom:1px solid ${C.hairline}}
  ...
  @media(max-width:768px){
    ...
    nav>div>div:last-child button{padding:8px 12px !important;font-size:13px !important}
    
    /* Hero section */
    section{padding:40px 20px !important}
    h1.serif{font-size:36px !important;line-height:1.1 !important}
    section>p{font-size:15px !important}
    section>div{flex-direction:column !important;gap:10px !important}
    section>div>button{width:100% !important;justify-content:center !important}
    
    /* Stats bar */
    section+section>div{grid-template-columns:repeat(2,1fr) !important;gap:0 !important}
    section+section>div>div{padding:20px 12px !important;border-right:none !important;border-bottom:1px solid rgba(255,255,255,.07) !important}
    ...
    .sidebar{width:100% !important;height:58px !important;flex-direction:row !important;position:fixed !important;bottom:0 !important;top:auto !important;z-index:200 !important;border-right:none !important;border-top:1px solid rgba(255,255,255,.1) !important;padding:0 !important}
    .sidebar>div:first-child{display:none !important}
    .sidebar>div:last-child{display:none !important}
    .sidebar nav{flex-direction:row !important;padding:0 !important;flex:1 !important;overflow:hidden !important}
    .ni{flex-direction:column !important;font-size:9px !important;gap:3px !important;padding:6px 8px !important;border-left:none !important;border-top:2px solid transparent !important;flex:1 !important;justify-content:center !important;align-items:center !important;min-width:0 !important}
    .ni.on{border-left:none !important;border-top-color:#4ade80 !important;background:#1a2a1a !important}
    .ni span{white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:56px}
    .main{margin-bottom:58px !important;overflow-y:auto !important}
    ...
    .contacts{width:100% !important;max-height:40vh !important;border-right:none !important;border-bottom:1px solid #E0DDD5 !important}
    
    /* Content padding */
    .fi>div{padding:16px !important}
    ...
    /* Modal */
    .modal{width:calc(100vw - 24px) !important;padding:20px !important;margin:12px !important}
    
    /* Auth forms */
    div[style*="width:420px"]{width:calc(100vw - 32px) !important;margin:0 16px !important}
    div[style*="width:400px"]{width:calc(100vw - 32px) !important;margin:0 16px !important}
  }
  
  @media(max-width:480px){
    h1.serif{font-size:28px !important}
    .ni svg{width:18px !important;height:18px !important}
  ...
  .bubble-me{background:${C.accent};color:#fff;border-radius:18px 18px 4px 18px;padding:10px 16px;font-size:14px;line-height:1.6;word-break:break-word;max-width:65%;display:inline-block;text-align:left}
  .bubble-them{background:${C.surface};border:1px solid ${C.hairline};color:${C.ink};border-radius:18px 18px 18px 4px;padding:10px 16px;font-size:14px;line-height:1.6;word-break:break-word;max-width:65%;display:inline-block;text-align:left}
    ...
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:C.bg}}>
      ...
      <div style={{width:420}}>
        ...
        <div className="card" style={{padding:28}}>
          ...
          {btnLabel!=="Log in"&&<div style={{display:"flex",alignItems:"flex-start",gap:10,marginBottom:16,padding:"12px",background:C.surfaceAlt,borderRadius:8}}>
            <input type="checkbox" id="tnc" style={{marginTop:2,flexShrink:0,accentColor:C.accent}} onChange={e=>setForm(p=>({...p,tnc:e.target.checked}))}/>
            <label htmlFor="tnc" style={{fontSize:12,color:C.muted,lineHeight:1.6,cursor:"pointer"}}>
              I agree to the <a href="#" onClick={e=>{e.preventDefault();setShowTnc(true)}} style={{color:C.accent,fontWeight:600}}>Terms & Conditions</a> and <a href="#" onClick={e=>{e.preventDefault();setShowTnc(true)}} style={{color:C.accent,fontWeight:600}}>Code of Conduct</a>. I understand that violating these terms may result in account termination and legal action.
            </label>
          </div>}
          <button className="bp" onClick={()=>{if(btnLabel!=="Log in"&&!form.tnc){alert("Please agree to the Terms & Conditions to continue.");return;}onSubmit(form)}} style={{width:"100%",justifyContent:"center",padding:"12px 20px",marginTop:4}}>
            {loading?<Spin/>:btnLabel}
          </button>
        </div>
        {showTnc&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.6)",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={()=>setShowTnc(false)}>
        <div style={{background:C.surface,borderRadius:16,padding:28,width:640,maxWidth:"calc(100vw - 32px)",maxHeight:"80vh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
            <h2 style={{fontSize:18,fontWeight:700}}>Terms & Conditions</h2>
            <button className="bg" onClick={()=>setShowTnc(false)} style={{padding:"5px 9px"}}><I n="x" size={15}/></button>
          </div>
          <div style={{fontSize:13,lineHeight:1.8,color:C.muted}}>
            <h3 style={{color:C.ink,marginBottom:8}}>1. Platform Usage</h3>
            <p style={{marginBottom:12}}>By registering on MyTutors24 you agree to use the platform only for legitimate educational purposes and to treat all other users with respect and professionalism.</p>
            <h3 style={{color:C.ink,marginBottom:8}}>2. Non-Circumvention (Tutors)</h3>
            <p style={{marginBottom:12}}>Tutors agree NOT to contact or accept payment from students outside the MyTutors24 platform for 12 months. Violation entitles MyTutors24 to claim damages equal to 3x the value of off-platform sessions.</p>
            <h3 style={{color:C.ink,marginBottom:8}}>3. Actions Affecting Company</h3>
            <p style={{marginBottom:12}}>Users must not make false or defamatory statements about MyTutors24, post fake reviews, encourage users to leave the platform, or take any action designed to damage MyTutors24 business reputation or financials. Violations may result in legal action.</p>
            <h3 style={{color:C.ink,marginBottom:8}}>4. Cancellation & Refunds</h3>
            <p style={{marginBottom:12}}>Cancellations more than 24 hours before session: full refund. Cancellations 2-24 hours before: 50% refund. Less than 2 hours: no refund. Tutor cancellations: full refund to student.</p>
            <h3 style={{color:C.ink,marginBottom:8}}>5. Professional Conduct</h3>
            <p style={{marginBottom:12}}>All users must maintain professional conduct. Harassment, discrimination, sharing student data, or academic dishonesty assistance will result in immediate permanent ban.</p>
            <h3 style={{color:C.ink,marginBottom:8}}>6. Payments</h3>
            <p style={{marginBottom:12}}>All payments must go through MyTutors24. Platform fee is 15% of session value. Tutors receive 85%. Refunds are processed within 5-7 business days.</p>
            <h3 style={{color:C.ink,marginBottom:8}}>7. Jurisdiction</h3>
            <p style={{marginBottom:20}}>These terms are governed by Indian law. Disputes are subject to courts in Hyderabad, Telangana.</p>
            <div style={{background:C.accentLight,padding:16,borderRadius:8,fontSize:12,color:C.accentText}}>
              By clicking "I agree" you acknowledge you have read, understood and agree to be bound by these Terms & Conditions. Violations may result in account termination and legal action.
            </div>
          </div>
          <button className="bp" onClick={()=>setShowTnc(false)} style={{width:"100%",justifyContent:"center",marginTop:20}}>I Understand</button>
        </div>
      </div>}
...
export default function App() {
  ...
  if(route==="student-register") return <AuthForm title="Create your account" sub="Join thousands of students learning."   fields={[{key:"name",label:"Full name",ph:"Rohan Malhotra"},{key:"email",label:"Email",type:"email",ph:"you@email.com"},{key:"password",label:"Password",type:"password",ph:"Min 6 characters"},{key:"city",label:"City",ph:"Delhi"},{key:"phone",label:"WhatsApp number",ph:"9876543210"}]} onSubmit={regStu}   loading={loading} error={err} footer={{text:"Already registered?",label:"Log in",fn:()=>go("student-login")}}   onBack={()=>go("login-selector")} btnLabel="Create account"/>
  ...
  if(route==="tutor-register")   return <AuthForm title="Join as a tutor"     sub="Start teaching on MyTutors24."          fields={[{key:"name",label:"Full name",ph:"Dr. Arjun Sharma"},{key:"email",label:"Email",type:"email",ph:"you@email.com"},{key:"password",label:"Password",type:"password",ph:"Min 6 characters"},{key:"subject",label:"Subject",type:"select",opts:SUBJ_LIST},{key:"city",label:"City",ph:"Delhi"},{key:"mode",label:"Mode",type:"select",opts:["Online","Offline","Hybrid"]},{key:"price",label:"Hourly rate (₹)",ph:"e.g. 1200"},{key:"phone",label:"WhatsApp number",ph:"9876543210"}]} onSubmit={regTut} loading={loading} error={err} footer={{text:"Already registered?",label:"Log in",fn:()=>go("tutor-login")}} onBack={()=>go("login-selector")} btnLabel="Apply as tutor"/>
